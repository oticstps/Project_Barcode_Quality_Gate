# =========================================================
# SISTEM POKA-YOKE INDUSTRIAL FINAL VERSION (PRODUCTION READY)
# Raspberry Pi + EVDEV + 3 Barcode Scanner (BY-PATH LOCKED)
# FULLSCREEN + UI + ERROR LOCK + S1 OVERRIDE + SPAM BYPASS + IGNORE UNKNOWN + ID CARD SAFE
# =========================================================
...
# =========================================================
# SCANNER DEVICE (Jalur Fisik Paten / By-Path)
# Mengunci urutan scanner secara permanen berdasarkan port USB fisik
# =========================================================
...
# =========================================================
# SETTING TOLERANSI DOUBLE-SCAN (DEBOUNCE)
# =========================================================
...
# =========================================================
AUDIO_READY = False
ALARM_CHANNEL, VOICE_CHANNEL, SOUND_ALARM, SOUND_SCAN_2_TIMEOUT = None, None, None, None

def load_sound(file_path):
    if not AUDIO_READY: return None
    if not file_path.exists(): return None
    try: return pygame.mixer.Sound(str(file_path))
    except: return None
...
def stop_all_sound():
    if AUDIO_READY: pygame.mixer.stop()

def play_alarm_loop():
    if not AUDIO_READY or SOUND_ALARM is None or ALARM_CHANNEL is None: return
    ALARM_CHANNEL.stop()
...
def timeout_alarm_worker():
    if not AUDIO_READY or SOUND_SCAN_2_TIMEOUT is None or SOUND_ALARM is None: return
    while not timeout_alarm_stop_event.is_set():
        ALARM_CHANNEL.stop()
        VOICE_CHANNEL.stop()
        VOICE_CHANNEL.play(SOUND_SCAN_2_TIMEOUT)
        while VOICE_CHANNEL.get_busy() and not timeout_alarm_stop_event.is_set(): time.sleep(0.05)
        if timeout_alarm_stop_event.wait(0.15): break
        ALARM_CHANNEL.play(SOUND_ALARM)
        while ALARM_CHANNEL.get_busy() and not timeout_alarm_stop_event.is_set(): time.sleep(0.05)
        if timeout_alarm_stop_event.wait(0.15): break
...
def start_timeout_alarm_sequence():
    ...
    timeout_alarm_thread = threading.Thread(target=timeout_alarm_worker, daemon=True)
    timeout_alarm_thread.start()

def stop_timeout_alarm_sequence():
    global timeout_alarm_thread
    timeout_alarm_stop_event.set()
    if AUDIO_READY and ALARM_CHANNEL: ALARM_CHANNEL.stop(); VOICE_CHANNEL.stop()
    if timeout_alarm_thread: timeout_alarm_thread.join(timeout=0.2)
    timeout_alarm_thread = None
...
def relay_blink_worker(pin):
    while not relay_blink_stop_event.is_set():
        ...
        if relay_blink_stop_event.wait(BLINK_INTERVAL_SECONDS): break
...
def start_relay_blink(pin):
    ...
    relay_blink_thread = threading.Thread(target=relay_blink_worker, args=(pin,), daemon=True)
    relay_blink_thread.start()

def stop_relay_blink():
    global relay_blink_thread
    relay_blink_stop_event.set()
    if relay_blink_thread: relay_blink_thread.join(timeout=0.2)
    relay_blink_thread = None
    if GPIO:
        GPIO.output(GPIO_13621, GPIO.LOW)
        GPIO.output(GPIO_13622, GPIO.LOW)
...
class PokaYokeSystem:
    ...
    def __init__(self, root):
        ...
        # Variabel Memori S1 (Menghindari Spam Statis)
        self.current_s1_barcode = ""
        
        # Variabel untuk Toleransi Pembacaan Ganda S2/S3 (Debounce)
        ...
        # THREAD
        threading.Thread(target=self.evdev_worker, args=(SCANNER_1_PATH, "S1"), daemon=True).start()
        threading.Thread(target=self.evdev_worker, args=(SCANNER_2_PATH, "S2"), daemon=True).start()
        threading.Thread(target=self.evdev_worker, args=(SCANNER_3_PATH, "S3"), daemon=True).start()

        self.write_log(f"SYSTEM READY. FULL PROTECTIONS AKTIF (ID Card Safe Mode).")
    ...
    def build_ui(self):
        ...
        header = tk.Frame(self.root, bg=self.COLOR_HEADER, height=78)
        ...
        info_frame = tk.Frame(status_frame, bg=self.COLOR_PANEL)
        ...
        scanner_frame = tk.Frame(self.root, bg=self.COLOR_BG)
        scanner_frame.pack(fill=tk.X, padx=24, pady=(0, 8))

        self.create_scanner_card(scanner_frame, "SCANNER 1", "Start Scanner").pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 8))
        self.create_scanner_card(scanner_frame, "SCANNER 2", "Judgment Line S2").pack(side=tk.LEFT, expand=True, fill=tk.X, padx=8)
        self.create_scanner_card(scanner_frame, "SCANNER 3", "Judgment Line S3").pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(8, 0))
        ...
        self.create_button(button_frame, "RESET UI", self.reset_ui).pack(side=tk.LEFT, padx=(0, 10))
        self.create_button(button_frame, "EXPORT CSV", self.export_csv).pack(side=tk.LEFT, padx=10)
        self.create_button(button_frame, "EXIT", self.close_program, danger=True).pack(side=tk.RIGHT)
    ...
    def create_scanner_card(self, parent, title, subtitle):
        ...
        line = tk.Frame(frame, bg="#334155", height=3)
    ...
    def start_scan_2_timeout(self):
        ...
        self.timeout_after_id = self.root.after(SCAN_2_TIMEOUT_SECONDS * 1000, self.handle_timeout_violation)

    def cancel_scan_2_timeout(self):
        if self.timeout_after_id:
            try: self.root.after_cancel(self.timeout_after_id)
            except: pass
            self.timeout_after_id = None

    def handle_timeout_violation(self):
        self.timeout_after_id = None
        if self.status != "WAIT_JUDGMENT": return
    ...
    def insert_table(self, scanner, barcode, phase, result):
        ...
        self.table.insert("", 0, values=(now, scanner, barcode, phase, result))
    ...
    def error_lock(self, text):
        ...
        self.label_status.config(text=text, bg=self.COLOR_STATUS_ERROR, fg="#ffffff")
    ...
    def process_barcode(self, barcode, scanner):
        ...
        # =================================================
        # -1. FILTER GLOBAL (SILENT IGNORE UNKNOWN BARCODE)
        # =================================================
        is_valid_part = (PRODUCT_13621 in barcode) or (PRODUCT_13622 in barcode)
        is_valid_reset = any(rc in barcode for rc in RESET_CODES)

        if not (is_valid_part or is_valid_reset):
            return

        # =================================================
        # -0.5. ID CARD SAFE-MODE (PENCEGAH ERROR SAAT NORMAL)
        # =================================================
        # Jika mesin TIDAK error, tapi yg discan adalah ID Card, langsung abaikan.
        # Diletakkan di sini agar ID Card tidak memicu Spam Filter jika di-scan berulang kali.
        if is_valid_reset and self.status != "ERROR_LOCK":
            # self.write_log(f"[{scanner}] Abaikan ID Card (Mesin Normal)") 
            # (Sengaja tidak di-print agar log tetap rapi kalau leader iseng nembak)
            return

        # =================================================
        # 0. BYPASS SPAM STATIS S1
        # =================================================
        if scanner == "S1" and barcode == self.current_s1_barcode:
            return

        # =================================================
        # 1. PENYARING (FILTER) TOLERANSI DOUBLE-SCAN S2/S3
        # =================================================
        ...
        if self.last_scanner == scanner and self.last_scan_data == barcode:
            if (current_time - self.last_scan_time) <= DOUBLE_SCAN_TOLERANCE_SECONDS:
                ...
                if self.spam_count <= MAX_SPAM_TOLERANCE:
                    self.write_log(f"[{scanner}] Abaikan spam ({self.spam_count}/{MAX_SPAM_TOLERANCE}).")
                    return
                else:
                    self.write_log(f"[{scanner}] SPAM MELEBIHI BATAS! Memicu Error.")
            else:
                self.spam_count = 0
        else:
            self.spam_count = 0
        ...
        if self.status == "ERROR_LOCK":
            if is_valid_reset:
                ...
                self.label_status.config(text="RESET BERHASIL", bg=self.COLOR_STATUS_ACTIVE, fg="#ffffff")
                self.root.after(1500, lambda: self.label_status.config(text="MENUNGGU SCAN AWAL", bg=self.COLOR_STATUS_WAIT, fg=self.COLOR_TEXT))
            else:
                self.write_log(f"AKSES DITOLAK : {barcode}")
            return

        # =================================================
        # 3. S1 OVERRIDE (Start Cycle / Deteksi Perubahan Part)
        # =================================================
        if scanner == "S1":
            detected_part = None
            ...
            if detected_part:
                ...
                self.label_status.config(text=f"SCAN DI {self.expected_scanner}", bg=self.COLOR_STATUS_ACTIVE, fg="#ffffff")
                self.label_target.config(text=f"Target Part : {detected_part}")
                self.label_mode.config(text="Mode : WAIT_JUDGMENT")

                self.insert_table(scanner, barcode, "SCAN_START", "OK")
                self.write_log(f"TARGET : {self.expected_scanner}")

                self.start_scan_2_timeout()

                target_pin = GPIO_13621 if detected_part == PRODUCT_13621 else GPIO_13622
                start_relay_blink(target_pin)
                return
            else:
                self.cancel_scan_2_timeout()
                self.insert_table(scanner, barcode, "SCAN_START", "NG")
                self.error_lock("ERROR ! PART INVALID")
                play_alarm_loop()
                return

        # =================================================
        # 4. JIKA S2 / S3 MENEMBAK SAAT MENUNGGU S1
        # =================================================
        ...
        # =================================================
        # 5. JIKA S2 / S3 MENEMBAK UNTUK JUDGMENT
        # =================================================
            ...
            if correct_scanner and correct_part:
                stop_relay_blink()
                self.insert_table(scanner, barcode, "SCAN_JUDGMENT", "OK")
                
                self.label_status.config(text="VALIDASI BERHASIL", bg=self.COLOR_STATUS_SUCCESS, fg="#ffffff")
                ...
                stop_relay_blink()
                self.insert_table(scanner, barcode, "SCAN_JUDGMENT", "NG")
                play_alarm_loop()
    ...
    def reset_to_wait(self):
        ...
        self.label_status.config(text="MENUNGGU SCAN AWAL", bg=self.COLOR_STATUS_WAIT, fg=self.COLOR_TEXT)
        self.label_target.config(text="Target Part : -")
        self.label_mode.config(text="Mode : WAIT_START")
    ...
    def export_csv(self):
        ...
        with open(CSV_FILE, "w", newline="", encoding="utf-8") as file:
            ...
            writer.writerow(["TIME", "SCANNER", "BARCODE", "PHASE", "RESULT"])
